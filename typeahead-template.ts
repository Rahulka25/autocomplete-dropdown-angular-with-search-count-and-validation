  ...
  private filter(term:any) {
    this.isToolTipActive = false;
      if (statesWithFlags.filter(v => v.name.toLowerCase().indexOf(term.toLowerCase()) > -1).length >4) {
        ...
        sateList.push({'name': '4 out of 50', 'flag': '5/5c/Flag_of_Alabama.svg/45px-Flag_of_Alabama.svg.png'});
        sateList.push({'name': 'type more for filtering', 'flag': '5/5c/Flag_of_Alabama.svg/45px-Flag_of_Alabama.svg.png'});
        return sateList;
      } else {
        let sateList = statesWithFlags.filter(v => v.name.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 4);
        sateList.push({'name': '4 out of 50', 'flag': '5/5c/Flag_of_Alabama.svg/45px-Flag_of_Alabama.svg.png'});
